...
interface IProps {
    ...
    loading : boolean;
    add_pre_production : (data : IPreProduction) => void;
    update_pre_production : (data : IPreProduction, id : string) => void;
    deactivate_pre_production : (id : string) => void;
    activate_pre_production : (id : string) => void;
    status : any
}

const PreProductionTable = React.forwardRef((props : IProps, ref) => {
    const getColumnSearch = useTableSearch();
    const getColumnFilterDate = useTableFilterDate();
    const [ form ] = Form.useForm();
    const [ data, setData ] = useState<Array<IPreProduction>>([]);
    ...
    useEffect(() => {
        const type = editingKey === 'new' ? 
            'PRE_PRODUCTION_ADD' : 'PRE_PRODUCTION_UPDATE';
        if(flag && props.status[type] ) {
            if (props.status[type].error === null) {
                notification.success({ 
                    message:  `Pre-Production successfully 
    ...
    const save = async () =>{
        try {
            setFlag(true);
            const data = (await form.validateFields()) as IPreProduction;
            if(editingKey === 'new'){
                props.add_pre_production(data);
            }else{
                props.update_pre_production(data, editingKey);
            }
            setEditingKey('');
        }catch (errInfo) {
    ...
    const statusHandler  = async (status : boolean, id : string) =>{
        if(!status){
            await props.deactivate_pre_production(id);
            notification.success({ 
                message:  `Pre-Production successfully deactivated`
            })
        }else {
            await props.activate_pre_production(id);
            notification.success({ 
                message:  `Pre-Production successfully activated`
            })
        }
    }

    const edit = (record: IPreProduction) => {
    ...
    const columns : Array<any>= [
        {
            ...
            dataIndex: 'pre_production_name',
            editable: true,
            key: 'pre_production_name',
            ...getColumnSearch(
                ['pre_production_name'], 
                (props) => <>{(props.val)}</>
            ),
...
export default PreProductionTable;
