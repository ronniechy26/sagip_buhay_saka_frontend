...
interface IProps {
    ...
    loading : boolean;
    add_production_stage : (data : IProductionStage) => void;
    update_production_stage : (data : IProductionStage, id : string) => void;
    deactivate_production_stage : (id : string) => void;
    activate_production_stage : (id : string) => void;
    status : any
}

const ProductionStageTable = React.forwardRef((props : IProps, ref) => {
    const getColumnSearch = useTableSearch();
    const getColumnFilterDate = useTableFilterDate();
    const [ form ] = Form.useForm();
    const [ data, setData ] = useState<Array<IProductionStage>>([]);
    ...
    useEffect(() => {
        const type = editingKey === 'new' ? 
            'PRODUCTION_STAGE_ADD' : 'PRODUCTION_STAGE_UPDATE';
    ...
    const save = async () =>{
        try {
            setFlag(true);
            const data = (await form.validateFields()) as IProductionStage;
            if(editingKey === 'new'){
                props.add_production_stage(data);
            }else{
                props.update_production_stage(data, editingKey);
            }
            setEditingKey('');
        }catch (errInfo) {
    ...
    const statusHandler  = async (status : boolean, id : string) =>{
        if(!status){
            await props.deactivate_production_stage(id);
            notification.success({ 
                message:  `Production Stage successfully deactivated`
            })
        }else {
            await props.activate_production_stage(id);
            notification.success({ 
                message:  `Production Stage  successfully activated`
            })
        }
    }

    const edit = (record: IProductionStage) => {
    ...
    const columns : Array<any>= [
        {
            ...
            dataIndex: 'production_stage_name',
            editable: true,
    ...
    const mergedColumns = columns.map(column => {
        ...
        return {
          ...column,
          onCell: (record: IProductionStage) => ({
            record,
...
export default ProductionStageTable;
